...
import { formatPossibilityCode } from '../../../shared/constants';

export default function ScreenshotUploader({ outcomePlanId, screenshots, tradingDay, onRefresh }) {
  const { addScreenshot, deleteScreenshot } = useTradingDay();
    ...
    for (const item of items) {
      if (item.type.startsWith('image/')) {
        ...
        try {
          ...
          await window.api.screenshot.create({
            outcomePlanId,
  ...
  return (
    <div tabIndex={0} onPaste={handlePaste} className="outline-none">
      ...
      {(!screenshots || screenshots.length === 0) && (
        ...
        >
          <svg className="w-10 h-10 text-surface-500 group-hover:text-primary-400/60 mx-auto mb-2 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2">
            {/* Candlestick chart */}
