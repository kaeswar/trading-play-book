...
// Single-screenshot uploader — one per kind (setup | outcome).
// When a screenshot exists: show it full-width with Replace + Remove controls.
// When empty: show dashed drop-zone (click or paste).
export default function SwingPlanScreenshotUploader({
  swingPlanId, screenshots, symbolName, planDate, onRefresh,
  kind = 'setup', label = 'Screenshot', disabled = false,
}) {
  const { addScreenshot, addScreenshotFromBuffer, deleteScreenshot } = useSwingPlan();
  const { showNotification } = useApp();
  const [uploading, setUploading] = useState(false);
  const [justUploaded, setJustUploaded] = useState(false);

  const existing = screenshots?.[0] ?? null;
  // Optimistically treat as "has screenshot" immediately after upload,
  // before the parent re-renders with the refreshed screenshots prop.
  const hasScreenshot = !!existing || justUploaded;

  const uploadFile = async (filePath) => {
    const fileName = filePath.split(/[/\\]/).pop();
    await addScreenshot(swingPlanId, filePath, symbolName, planDate, `${Date.now()}_${fileName}`, kind);
  };

  const uploadBuffer = async (uint8Array, fileName) => {
    await addScreenshotFromBuffer(swingPlanId, uint8Array, symbolName, planDate, fileName, kind);
  };
  ...
  const replaceWith = async (doUpload) => {
    ...
    try {
      ...
      await doUpload();
      setJustUploaded(true);  // optimistically flip to Replace/Remove before parent re-renders
      await onRefresh();
    } catch {
